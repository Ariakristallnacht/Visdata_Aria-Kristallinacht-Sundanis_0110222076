...
def multiple_scatter():
    ...
    fig, ax = plt.subplots()
    ax.scatter(suhu, penjualan_weekdays, color='green', label='Hari Kerja', s=100)
    ax.scatter(suhu, penjualan_weekend, color='purple', label='Akhir Pekan', s=100)
    ax.set_title('Hubungan Penjualan Es Krim dengan Suhu')
    ax.set_xlabel('Suhu')
    ax.set_ylabel('Penjualan Es Krim')
    ax.grid(True)
...
def scatter_3_variable():
    ...
    # logika untuk opsi jenis eskrim berdasarkan
